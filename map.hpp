...
public:
  ...
  iterator begin() {
    if (!_Avl_tree.getMinValNode())
      return iterator(_Avl_tree.getEndNode());
    return iterator(_Avl_tree.getMinValNode());
  }
  const_iterator begin() const {
    if (!_Avl_tree.getMinValNode())
      return const_iterator(_Avl_tree.getEndNode());
...
struct Iterator{
  ...
  Self  operator++() {
    _node = Avl_tree_increment<NodePtr>(_node);
    return *this;
  }
  Self operator++(int) {
    Self tmp = *this;
  ...
  Self  operator--() {
    _node = Avl_tree_decrement<NodePtr>(_node);
    return *this;
  }
  Self  operator--(int) {
    Self tmp = *this;
